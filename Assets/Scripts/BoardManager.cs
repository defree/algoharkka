...
using Random = UnityEngine.Random; 		//Tells Random to use the Unity Engine random number generator.
    ...
    public class BoardManagerTommin : MonoBehaviour
    {
            ...
            for (int x = map.GetLength(0) - 1; x >= 0; x--)
            {
                for (int y = map.GetLength(1) - 1; y >= 0; y--)
                {
                    ...
                    if (map[x, y] == 2)
                    {
                        ...
                        UnityEngine.Object o = Instantiate(tileChoice, positijoni, Quaternion.identity);
                        Enemy e = ((GameObject)o).GetComponent<Enemy>();
                        if (e != null)
                        {
							// RANDOMILLA ENEMYTYPE, eli Enemy.Type.Guard tai Enemy.Type.Patroller
                            e.enemyType = Enemy.Type.Guard;
                            e.HP = 3;
							//MIKÄLI ENEMYTYPE == Enemy.Type.Patroller, PITÄÄ ASETTAA
							// e.A = new Cell(x,y);
							// e.B = new Cell(x,y);
                        }
                    }
